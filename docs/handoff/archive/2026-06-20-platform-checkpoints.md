# 2026-06-20 Platform Handoff Archive

This file was split from docs/agent-handoff.md to keep the active handoff
entrypoint small and reduce branch merge conflicts. Read this archive only when
older platform, deployment, or verification history is needed.
...
- Latest task addendum (2026-06-19, P1.59 no-registry deployment path):
  ...
  - Verification:
    - Read the user's reference docs:
      `鑵捐浜戦儴缃插姞閫熸柟妗?md` and `娴佹按绾块儴缃叉祦绋?md`.
...
- Current architecture decision:
  ...
  - Main Codex owns architecture/integration/verification; `coding` owns
    bounded implementation; `鍒ゅ畼` owns review; `闃庣帇` is reserved for high-level
    tradeoff work; `鐗涘ご` owns Claude Code / `claude-ds` packets only when
    server-side work is useful.
  - Agent lifecycle rule: maintain only four logical roles (`coding`, `闃庣帇`,
    `鍒ゅ畼`, `鐗涘ご`), reuse existing role agents instead of spawning per
    iteration, close completed runtime agents after integration, and reserve
    `闃庣帇` / `gpt-5.5` for architecture tradeoffs only.
  - Agent naming rule: ignore random runtime nicknames in coordination; all
    ...
    stable role names (`coding`, `闃庣帇`, `鍒ゅ畼`, `鐗涘ご`). Start each delegated
    prompt with `浣犵殑瑙掕壊鍚嶆槸 <role>`.
