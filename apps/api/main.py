...
async def validation_exception_handler(request: Request, exc: RequestValidationError):
    ...
    # Log individual field errors at debug level for troubleshooting.
    for detail in validation_details:
        loc = ".".join(str(p) for p in detail["loc"])
        logger.debug(
            f"request_id={get_request_id()} "
            f"validation_error loc={safe_log_value(loc, max_length=120)} "
            f"msg={safe_log_value(detail['msg'])}"
        )
    return JSONResponse(
        status_code=422,
        content=fail(
